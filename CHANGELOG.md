...
- **Coverage build — `undefined reference to __gcov_init`**: `microservice_domain` and `microservice_infra` were compiled with `-fprofile-arcs -ftest-coverage` when `ENABLE_COVERAGE=ON`, but `microservice_app` had no coverage block. The gcov instrumentation in the static library objects calls `__gcov_init`/`__gcov_exit`/`__gcov_merge_add`, which the gcov runtime (`--coverage` link flag) provides. Fixed by adding the identical `if(ENABLE_COVERAGE)` block to `src/application/CMakeLists.txt`.
- **CTest "No tests were found"**: `enable_testing()` was only called inside `tests/CMakeLists.txt`. CMake docs state it must be in the source root so that `ctest --test-dir build/Release` finds the root-level `CTestTestfile.cmake`. Without it, CTest scanned the root build directory, found no `add_test()` entries, and reported zero tests even though the subdirectory registrations existed. Fixed by adding `enable_testing()` to the root `CMakeLists.txt`.
- **Test link failure — `undefined reference to test_main` (definitive fix)**: Reverting `Drogon::Drogon` to `PRIVATE` on `microservice_infra` was necessary but not sufficient. CMake's rule for static libraries propagates ALL link dependencies (PRIVATE and PUBLIC) to every consumer's final link command, because the archive's object files must have their symbols resolved. The HTTP controller objects in `microservice_infra` reference Drogon symbols, so the linker continued to drag in Drogon → Boost → `libboost_test_exec_monitor.a`. Fixed by removing `microservice_infra` from `microservice_tests` entirely: `GreetingUseCaseTest.cpp` needs only `microservice_domain`; `SpdlogLoggerTest.cpp` needs only `SpdlogLogger.cpp` + `spdlog::spdlog`. `SpdlogLogger.cpp` is now compiled directly into the test binary.
